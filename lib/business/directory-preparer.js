...
export function prepareProblemDirectory(problemId, snippet, activateEditor, closeAfterOpening) {
...
export function prepareContestDirectory(contestId) {
    ...
    if (problems && problems.length > 0) {
        ...
        problems.slice(1).reverse().forEach(problem => {
            prepareProblemDirectory(problem.id, snippet, false, false);
        });
    }
    ...
    backgroundProblems.forEach(problem => {
        prepareProblemDirectory(problem.id, snippet, false, true);
    })
}
