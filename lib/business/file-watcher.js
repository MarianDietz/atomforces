...
function changed(p) {
    ...
    const problemDirectory = ConfigStore.getFileStructureProblemDirectory()({ problem: problemIndex });
    if (path.relative(rel, path.join(problemDirectory, ConfigStore.getFileStructureSourceFileName()({ problem: problemIndex }))) === '') {
        updateCompilation(problemId, { outdated: true });
        if (ConfigStore.getAutoCompilation()) {
            compile(problemId);
        }
    } else if (path.relative(rel, path.join(problemDirectory, ConfigStore.getFileStructureExecutableName()({ problem: problemIndex }))) == '') {
        outdateAllExamples(problemId);
        const problem = ProblemStore.getProblems().get(problemId);
        if (ConfigStore.getAutoEvaluation() && problem && !problem.compilation.killProcess) {
            runAllExamples(problemId);
        }
    } else {
        fileChanged(problemId, p, path.relative(problemDirectory, rel));
    }
}
