...
export default class ContestUpdater {
            ...
            setProblems(this.contestId, new Map(problems
                ...
                .map(problem => {
                    return [problem.index, {
                        index: problem.index,
                        name: problem.name,
                        filePath: path.join(contest.filePath, problem.index)
                    }];
                })
            ));
        ...
        Array.from(contest.problems.values()).forEach(problemId => {
            ...
            promises.push(CodeforcesScraper.getSamples(contest.codeforcesId, problem.index).then(examples => {
                ...
                examples.forEach(([input, output]) => {
                    ...
                    const inPath = path.join(problem.filePath, 'examples', `${nextIndex}.in`);
                    const outPath = path.join(problem.filePath, 'examples', `${nextIndex}.out`);
    ...
    prepareContestDirectory() {
        ...
        const scope = atom.config.get('atomforces.templateSnippet.scope');
        const prefix = atom.config.get('atomforces.templateSnippet.prefix');
        var snippet = null;
    ...
    prepareProblemDirectory(problemId, snippet, activateEditor) {
        ...
        const sourceFilePath = path.join(problem.filePath, `${problem.index}.cpp`);
        atom.workspace.open(sourceFilePath, { split: 'up', activateItem: activateEditor }).then(editor => {
            ...
            fs.makeTree(path.join(problem.filePath, 'examples'));
        }).catch(error => {
            handleError(error);
        });
    }
