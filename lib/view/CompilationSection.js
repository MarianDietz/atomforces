...
export default class CompilationSection extends React.Component {
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        if (this.props.compilation.output && this.props.compilation.output.length > 0 && !this.props.compilation.outdated) {
    ...
    render() {
        ...
        return <CollapsableSubsection collapsed={this.state.collapsed} toggleCollapsed={this.toggleCollapsed}
                heading={<span><span style={{marginRight: '1em'}}>Compilation</span> {companion}</span>}>
